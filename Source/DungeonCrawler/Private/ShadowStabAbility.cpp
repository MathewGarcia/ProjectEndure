...
void UShadowStabAbility::Logic()
{
	if(AEnemy*Enemy = Cast<AEnemy>(GetOuter()))
	{
		if(UMainGameInstance*MainGI = Cast<UMainGameInstance>(Enemy->GetGameInstance()))
		{
			...
			params.Owner = Enemy;
			if(ADebuffActor*DebuffActor = Enemy->GetWorld()->SpawnActor<ADebuffActor>(ActorToSpawn,Location,FRotator(0.f),params))
			{
				Enemy->GetWorldTimerManager().SetTimer(FCooldownTimer, [this]() {}, CooldownTimer, false);
			}
		}
	}
}
