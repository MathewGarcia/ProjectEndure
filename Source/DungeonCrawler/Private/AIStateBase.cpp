...
void AIStateBase::OnEnterState()
{
    ...
    if (AIEnemyController && world) {
        // Fix: Only create TWeakObjectPtr if AIEnemyController is not null
        TWeakObjectPtr<AAIEnemy> SafeAIEnemyController;
        if (AIEnemyController) {
            SafeAIEnemyController = AIEnemyController;
        } else {
            return;
        }
        world->GetTimerManager().SetTimer(ExecuteTimerHandle, [this,SafeAIEnemyController]()
            {
                ...
                if (!LocalAIEnemyController) return;
...
{
    if (world && world->GetTimerManager().IsTimerActive(ExecuteTimerHandle))
    {
            ...
            world->GetTimerManager().SetTimer(ExecuteTimerHandle, [this,SafeAIEnemyController]()
                {
                    ...
                    if (!LocalEnemyController) return;
