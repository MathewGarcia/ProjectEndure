...
void UPoisonDebuff::ExecuteDebuff_Implementation(AActor* Target, AActor* DamageCauser)
{
	...
	if (APlayerCharacter* player = Cast<APlayerCharacter>(Target))
	{
		...
		GetWorld()->GetTimerManager().SetTimer(FInnerTimerHandle, [safePlayer]()
			{
		...
		GetWorld()->GetTimerManager().SetTimer(FDebuffTimerHandle, [SafeDebuff,safePlayer, NiagaraComponent]()
			{
	...
	else if (AEnemy* enemy = Cast<AEnemy>(Target))
	{
		...
		TWeakObjectPtr<UPoisonDebuff> SafeDebuff = this;
		GetWorld()->GetTimerManager().SetTimer(FInnerTimerHandle, [safeEnemy]()
			{
		...
		GetWorld()->GetTimerManager().SetTimer(FDebuffTimerHandle, [SafeDebuff, safeEnemy, NiagaraComponent]()
			{
				...
				if (LocalDebuff->GetWorld()->GetTimerManager().IsTimerActive(LocalDebuff->FInnerTimerHandle))
				{
...
void UPoisonDebuff::EndBuff()
{
	...
	GetWorld()->GetTimerManager().ClearTimer(FInnerTimerHandle);
