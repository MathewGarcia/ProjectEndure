...
void UAoE_Heal::Logic()
{
	...
	if(ActorToSpawn)
	{
		if (APlayerCharacter* player = Cast<APlayerCharacter>(GetOuter()))
		{
			...
			if(AAoEHealActor*AoEHealActor = player->GetWorld()->SpawnActorDeferred<AAoEHealActor>(ActorToSpawn,player->GetActorTransform(),nullptr,nullptr, ESpawnActorCollisionHandlingMethod::AdjustIfPossibleButAlwaysSpawn))
			{
				...
				player->GetWorldTimerManager().SetTimer(FLifeCycle, [this,AoEHealActor]()
					{
