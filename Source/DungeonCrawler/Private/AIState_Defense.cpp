...
TUniquePtr<AIDecisionTreeNode> AIState_Defense::BuildDecisionTree(AEnemy* Enemy)
{
	...
	//if the player is in range, move backward, try to dodge away from the player. Take open opportunity attacks.
 ...
 */
	...
	if (APlayerCharacterState* PCS = Cast<APlayerCharacterState>(player->GetPlayerState()))
	{
		...
		TUniquePtr<AIDecisionTreeNode> Dodge = DecisionTreeHelper::MakeLeafNode([Enemy]() {Enemy->Dodge(FMath::RandRange(1, 3)); });
		TUniquePtr<AIDecisionTreeNode> AttackR = DecisionTreeHelper::MakeLeafNode([this]() {AIEnemyController->ChangeAIState(AIEnemyController->AttackState.Get()); });
		TUniquePtr<AIDecisionTreeNode> AttackL = DecisionTreeHelper::MakeLeafNode([this]() {AIEnemyController->ChangeAIState(AIEnemyController->AttackState.Get()); });
		TUniquePtr<AIDecisionTreeNode> MontageAttack = DecisionTreeHelper::MakeLeafNode([this]() {AIEnemyController->ChangeAIState(AIEnemyController->AttackState.Get()); });
		...
		TUniquePtr<AIDecisionTreeNode> AttackOverHalfHealth = DecisionTreeHelper::MakeLeafNode([this]() {AIEnemyController->ChangeAIState(AIEnemyController->AttackState.Get()); });
...
TUniquePtr<AIDecisionTreeNode> AIState_Defense::BuildNotInRange(AEnemy* Enemy)
{
	...
	if (APlayerCharacterState* PCS = Cast<APlayerCharacterState>(player->GetPlayerState()))
	{
		...
		TUniquePtr<AIDecisionTreeNode> Attack = DecisionTreeHelper::MakeLeafNode([this]() {AIEnemyController->ChangeAIState(AIEnemyController->AttackState.Get()); });
...
TUniquePtr<AIDecisionTreeNode> AIState_Defense::BuildBlock(AEnemy* Enemy)
{
	...
	TUniquePtr<AIDecisionTreeNode> Root = DecisionTreeHelper::MakeLeafNode([Enemy]() {Enemy->Dodge(FMath::RandRange(1, 3)); });
