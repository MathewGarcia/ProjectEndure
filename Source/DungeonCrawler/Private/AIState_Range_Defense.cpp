...
TUniquePtr<AIDecisionTreeNode> AIState_Range_Defense::BuildInRange(AEnemy* Enemy)
{
    ...
    TUniquePtr<AIDecisionTreeNode> SetToAttack = DecisionTreeHelper::MakeLeafNode([this]() {if (AIEnemyController) AIEnemyController->ChangeAIState(AIEnemyController->AttackState.Get()); });
...
{
    if (!AIEnemyController) return nullptr;
    TUniquePtr<AIDecisionTreeNode> SetToAttack = DecisionTreeHelper::MakeLeafNode([this]() {if (AIEnemyController) AIEnemyController->ChangeAIState(AIEnemyController->AttackState.Get()); });
    return SetToAttack;
}
