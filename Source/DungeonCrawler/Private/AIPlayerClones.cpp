...
void AAIPlayerClones::Execute()
{
    ...
    if (APlayerCharacter* OwningPawn = Cast<APlayerCharacter>(GetPawn()))
    {
        ...
        FVector DirectionToPlayer = ToPlayer.IsNearlyZero() ? FVector::ZeroVector : ToPlayer.GetSafeNormal();
...
void AAIPlayerClones::Tick(float DeltaSeconds)
{
    ...
    // No timers to manage here, Execute() is safe.
    Execute();
}
