...
TUniquePtr<AIDecisionTreeNode> AIState_Imp_Defense::BuildDecisionTree(AEnemy* Enemy)
{
    ...
    if (ShouldBuffAllies_L) ShouldBuffAllies_L->LeftChild = MoveTemp(InRange);
    if (ShouldBuffAllies_R) ShouldBuffAllies_R->LeftChild = MoveTemp(NotInRange);
    if (Root) {
...
TUniquePtr<AIDecisionTreeNode> AIState_Imp_Defense::BuildBuffAllies(AEnemy* Enemy)
{
    ...
    TUniquePtr<AIDecisionTreeNode> BuffAllies = DecisionTreeHelper::MakeLeafNode([Enemy]() {if (Enemy) Enemy->BuffFriendlies(); });
