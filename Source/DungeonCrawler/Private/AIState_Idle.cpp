...
void AIState_Idle::OnExitState()
{
    ...
    if (AEnemy* Enemy = Cast<AEnemy>(AIEnemyController ? AIEnemyController->GetPawn() : nullptr)) {
    ...
    if (AIEnemyController)
    {
        if (world)
        {
            if (AEnemy* Enemy = Cast<AEnemy>(AIEnemyController->GetPawn()))
            {
                ...
                FVector DirToPlayer = ToPlayer.IsNearlyZero() ? FVector::ZeroVector : ToPlayer.GetSafeNormal();
