	...
	UPrimitiveComponent* OtherComp, int32 OtherBodyIndex, bool bFromSweep, const FHitResult& SweepResult)
{
	...
	if (OtherActor && OtherActor != this && OtherActor != GetOwner() && !OtherActor->IsAttachedTo(this) && OverlappedComponent->GetOwner() != GetOwner())
	{
...
{
	//this will change to the actual camera direction the player is aiming at.
			...
			if (AWeapon* Weapon = player->GetAttackingWeapon()) {
				for (AActor* Actor : HitActors)
				{
					if (AEnemy* Enemy = Cast<AEnemy>(Actor))
					{
						...
						Enemy->TakeDamage(FinalDamage, FDamageEvent(), GetOwner()->GetInstigatorController(), GetOwner());
					}
				}
			}
		}
