                ...
                if (!bIsAbilityAlreadyLearned(Ability, PCS->LearnedAbilities)) {
                    ...
                    if (Ability->AbilityActivationType == EAbilityActivationType::Event)
                    {
                        ...
                        for (int i = 0; i < PCS->EquippedAbilities.Num(); ++i) {
                            if (PCS->EquippedAbilities[i] && PCS->EquippedAbilities[i]->AbilityTalentRow == AbilityRow)
                            {
                        ...
                        for (int i = 0; i < PCS->EquippedAbilities.Num(); ++i) {
                            ...
                            if (PCS->EquippedAbilities[i] && PCS->EquippedAbilities[i]->GetClass() == UEmptyAbility::StaticClass()) {
                                PCS->EquippedAbilities[i] = Ability;
                                break;
                            }
                        }
...
void UTalentWidget::NativeConstruct()
{
    ...
    if (AbilityClass) {
        ...
        {
            if (TalentImage && AbilityInfo->Image) {
...
{
    if (!CheckedAbility) return false;
    for(UAbility*Ability : LearnedAbilities)
    {
        if(Ability && Ability->GetClass() == CheckedAbility->GetClass())
        {
            return true;
        }
    }
...
void UTalentWidget::ChangeHoveredText()
{
    ...
    if (UTalentTreeWidget* TTW = OwningWidget.Get()) {
        if (TTW->DescriptionText && AbilityClass) {
