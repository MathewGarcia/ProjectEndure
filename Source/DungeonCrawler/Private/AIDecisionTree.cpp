...
float AIDecisionTree::calculateRawProbability(float k, AIDecisionTreeNode* currentNode, float featureValue, float IntelligencePenalty)
{
	...
	{
		// Deterministic: if featureValue is below the split, go left (probability 1), else go right (probability 0).
		return (featureValue < currentNode->SplitValue) ? 1.0f : 0.0f;
	}

	// For nonzero IntelligencePenalty, adjust the steepness of the logistic function.
	// Here we interpolate k: when IntelligencePenalty is 0, use a very steep slope (e.g., 10), 
	// and when it is 1, use the original k value (e.g., 1).
