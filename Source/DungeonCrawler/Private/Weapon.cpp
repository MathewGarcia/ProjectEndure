...
{
    bIsSwinging = true;
    if (bIsSkeletal && SkeletalMeshComponent) {
        StartLocation = SkeletalMeshComponent->GetComponentLocation();
...
void AWeapon::MoveWeapon(float DeltaTime)
{
    ...
    if(InterpAlphaVal >= 1.f)
    {
        ...
        UWorld* World = GetWorld();
        if (!World) return;
        if (World->SweepSingleByObjectType(Hit, StartLocation, CurrentWeaponLocation, Quat, ObjectQueryParams, FCollisionShape::MakeCapsule(Radius, Height), params))
        {
            if (AEnemy* Enemy = Cast<AEnemy>(Hit.GetActor()))
            {
                ...
                Enemy->TakeDamage(Damage, FDamageEvent(), GetOwner() ? GetOwner()->GetInstigatorController() : nullptr, GetOwner());
                TArray<AActor*> InnerActorsToIgnore;
                InnerActorsToIgnore.Add(this);
                InnerActorsToIgnore.Add(Enemy);
                TArray<AActor*> OutActors;
                if (UKismetSystemLibrary::SphereOverlapActors(World, GetActorLocation(), 1000.f, ObjectTypes, AEnemy::StaticClass(), InnerActorsToIgnore, OutActors))
                {
                    AEnemy* ClosestEnemy = nullptr;
                    UKismetSystemLibrary::DrawDebugSphere(World, GetActorLocation(), 1000.f, 12, FLinearColor::White, 123, 10);
...
void AWeapon::ActivateWeapon()
{
    ...
    UWorld* World = GetWorld();
    if (!World) return;
    if (World->SweepMultiByObjectType(Hits, StartLocation, CurrentWeaponLocation, Quat, ObjectQueryParams, FCollisionShape::MakeCapsule(Radius, Height), params))
    {
        for (auto Hit : Hits)
        {
            if (AEnemy* Enemy = Cast<AEnemy>(Hit.GetActor()))
                ...
                for (auto Hit : Hits)
                {
                    ...
                    else if (AEnvironmentalItem* EnvironmentalItem = Cast<AEnvironmentalItem>(Hit.GetActor()))
                    {
                        if (EnvironmentalItem->ActorHasTag("Destructible")) {
                            EnvironmentalItem->TakeDamage(100.f, FDamageEvent(), GetOwner() ? GetOwner()->GetInstigatorController() : nullptr, GetOwner());
                            if (EnvironmentalItem->NiagaraSystemToPlay && EnvironmentalItem->Mesh && EnvironmentalItem->Mesh->GetStaticMesh()) {
                                UNiagaraComponent* NiagaraComponent = UNiagaraFunctionLibrary::SpawnSystemAtLocation(World, EnvironmentalItem->NiagaraSystemToPlay, EnvironmentalItem->GetActorLocation());
                                if (NiagaraComponent)
                                {
